...
**Main paths (non-exhaustive):** `/` (landing), `/sign-in`, `/sign-up`, `/verify-email`, `/reset-password`, `/onboarding`, `/dashboard`, `/workspace`, `/campaigns`, `/campaign/:id`, `/generate`, `/products`, `/customer-data`, `/customer-data/all-consumers`, `/settings`, `/profile`, marketing pages (`/features`, `/pricing`, …). **`/all-consumers`** redirects to **`/customer-data/all-consumers`**.
