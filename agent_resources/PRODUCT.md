...
4. **Explore creative (preview)** — Call **campaign preview** generation: when the approved plan includes fenced JSON with **`persona_groups`**, resolve names to **`dbo.personas`**, take up to **`variants_per_group`** distinct consumers per group (from snapshot preferences when set), and generate **preview** variants (`is_preview=true`). If the plan has no usable JSON block, legacy behavior samples up to **six** random personas (tenant-scoped consumers). See `generate_campaign_preview` in `workers/ad_job_worker/worker.py`.
