...
| **Meta Marketing API** | `httpx` / Graph calls via `services/ad_platforms/meta/` | OAuth (`routes/social_auth.py`); insights (`services/ad_platforms/meta/insights.py`, `routes/metrics.py`); publish (`campaign_publisher.py`, **`PATCH /campaigns/{id}/run`**) | `META_*`, `FERNET_SECRET_KEY`, stored token on `SocialConnection` |
